function Get-Drives {
    ...
    if ($Mode -eq "all") {
        $driveTitles = @() # Initialize an array to collect drive information
        ...
        for ($i = 0; $i -lt $volumes.Count; $i++) {
            ...
            if ($thisDrive.FileSystemLabel -ne "System Reserved") {
                ...
                $driveTitles += $driveTitle # Add drive title to the array
            }
        }
        
        # Return all drive titles as a single output
        return $driveTitles -join "; " # Join with a separator (optional)
    }
    ...
    elseif ($Index -ge 0 -and $Index -lt $volumes.Count) {
        ...
        if ($selectedDrive.PSObject.Properties[$Property]) {
            ...
            return "Property '$Property' not found on the drive object."
        }
    }
    else {
        return "Invalid parameters. Please check the drive index and property."
    }
}
